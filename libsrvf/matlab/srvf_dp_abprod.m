...
function E = srvf_dp_abprod(Q1, T1, Q2, T2, a, i0, j0, i1, j1)
...
theta = real(acos(dot(ug,vg)./(sqrt(dot(ug,ug)).*sqrt(dot(vg,vg)))));
W = (sqrt(dot(ug,ug)).*sqrt(dot(vg,vg))).*((a*theta<=pi/2).*cos(a*theta));
