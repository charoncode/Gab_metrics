...
for j = 1:N
    ...
    Q2n = SRV_transform(c2n, T2);
    [G1, GT1, G2, GT2] = optimal_reparam(c1, T1, c2n, T2, method, a);

    % Apply optimal matching
    c1new = Group_Action_by_Gamma_Coord(c1,G1);
    c2new = Group_Action_by_Gamma_Coord(c2n,G2);
    Q1n = SRV_transform(c1new, T1);
    Q2n = SRV_transform(c2new, T2);
...
% Apply optimal matching
c1 = plf_evaluate(c1,T1,G1);
c2 = plf_evaluate(c2,T2,G2);


Tnew = linspace(0,1,size(G1,2));
c1new = c1;
T1new = Tnew;
c2new = c2;
T2new = Tnew;
